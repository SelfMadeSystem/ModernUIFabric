...
/**
 * CanvasFabric is an extension to {@link Canvas}, which provides more drawing
 * methods used in Minecraft on UI thread.
 *
 * @author BloCamLimb
 */
public final class CanvasFabric {

    private static final CanvasFabric sInstance = new CanvasFabric();
    ...
    /**
     * Get a CanvasFabric from the given Canvas, if available.
     *
     * @return the instance
     * @throws IllegalArgumentException no capability found with the canvas
     */
    @Nonnull
    public static CanvasFabric get(@Nonnull Canvas canvas) {
        if (sInstance.mCanvas == null) {
            synchronized (CanvasFabric.class) {
                if (sInstance.mCanvas == null) {
