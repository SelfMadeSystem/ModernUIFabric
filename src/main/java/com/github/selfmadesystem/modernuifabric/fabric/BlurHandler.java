...
import com.github.selfmadesystem.modernuifabric.fabric.mixin.AccessPostChain;
    ...
    void blur(@Nullable Screen nextScreen) {
        ...
        if (hasScreen && !mHasScreen) {
            if (!blocked && sBlurEffect && !mBlurring && gr.currentEffect() == null && sBlurRadius > 1) {
                try {
                    loadEffectMethod.invoke(gr, BLUR_POST_EFFECT);
...
//                gr.loadEffect(BLUR_POST_EFFECT);
    ...
    void loadBlacklist(@Nullable List<? extends String> names) {
        ...
        for (String s : names) {
            ...
            try {
                Class<?> clazz = Class.forName(s, false, ModernUIFabric.class.getClassLoader());
