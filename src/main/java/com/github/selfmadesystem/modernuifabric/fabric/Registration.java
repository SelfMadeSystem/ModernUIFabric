...
import com.github.selfmadesystem.modernuifabric.fabric.mixin.AccessOptions;
...
//        if (ModList.get().getModContainerById("kiwi").isPresent()) { // wtf why kiwi lol
...
//        }
        ...
        // give it a probe
        if (MuiFabricApi.isServerStarted()) {
            LOGGER.info(MARKER, "");
        }
    }
                ...
                (caption, value) -> {
                    if (value == 0) {
                        return Options.genericValueLabel(caption, Component.translatable("options.guiScale.auto")
                                .append(Component.literal(" (" + (MuiFabricApi.calcGuiScales() >> 4 & 0xf) + ")")));
                    } else {
                        MutableComponent valueComponent = Component.literal(value.toString());
                        if (value < (MuiFabricApi.calcGuiScales() >> 8 & 0xf)) {
    ...
    static class GuiScaleValueSet implements OptionInstance.IntRangeBase,
        ...
        public int maxInclusive() {
            return MuiFabricApi.calcGuiScales() & 0xf;
        }
