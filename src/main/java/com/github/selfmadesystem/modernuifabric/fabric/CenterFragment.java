...
import com.github.selfmadesystem.modernuifabric.fabric.Config.Client;
...
import com.github.selfmadesystem.modernuifabric.textmc.ModernUITextMC;
    ...
    private View createLeftPanel() {
...

        {
            ...
            }
            {
                ...
                var button = option.<SwitchButton>requireViewById(R.id.button1);
                button.setChecked((ModernUIFabric.getBootstrapLevel() & ModernUIFabric.BOOTSTRAP_DISABLE_SMOOTH_SCROLLING) == 0);
                button.setOnCheckedChangeListener((__, checked) -> {
                    int level = ModernUIFabric.getBootstrapLevel();
                    if (checked) {
                        level &= ~ModernUIFabric.BOOTSTRAP_DISABLE_SMOOTH_SCROLLING;
                    } else {
                        level |= ModernUIFabric.BOOTSTRAP_DISABLE_SMOOTH_SCROLLING;
                    }
                    ModernUIFabric.setBootstrapLevel(level);
...

        {
            ...
            var category = createCategory("modernui.center.category.text");
            {
                ...
                var button = option.<SwitchButton>requireViewById(R.id.button1);
                button.setChecked((ModernUIFabric.getBootstrapLevel() & ModernUIFabric.BOOTSTRAP_DISABLE_TEXT_ENGINE) == 0);
                button.setOnCheckedChangeListener((__, checked) -> {
                    int level = ModernUIFabric.getBootstrapLevel();
                    if (checked) {
                        level &= ~ModernUIFabric.BOOTSTRAP_DISABLE_TEXT_ENGINE;
                    } else {
                        level |= ModernUIFabric.BOOTSTRAP_DISABLE_TEXT_ENGINE;
                    }
                    ModernUIFabric.setBootstrapLevel(level);
